document
    ...
    const response = await fetch(`http://localhost/api/weather?city=${city}`);
...
async function fetchWeatherByCoords(lat, lon) {
    ...
    const current = data.list[0];
    document.getElementById("cityName").textContent = data.city.name;
    document.getElementById("temperature").textContent = `気温：${current.main.temp.toFixed(1)}℃`;
    document.getElementById("description").textContent = `天気：${current.weather[0].description}`;
    document.getElementById("weatherIcon").style.display = "block";
    document.getElementById("weatherIcon").src = `https://openweathermap.org/img/wn/${current.weather[0].icon}@2x.png`;

    const forecasts = [data.list[0], data.list[8], data.list[16], data.list[24], data.list[32]];
    const forecastList = document.getElementById("forecastList");
    forecastList.innerHTML = "";
